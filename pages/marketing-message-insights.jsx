...
const MarketingMessageInsights = () => {
  const marketingMessageInsightsData = useSelector(
    (state) => state.marketingMessageInsights,
  );
  const error = useSelector((state) => state.error.marketingMessageInsights);
  const labels = marketingMessageInsightsData.map((data) =>
    formatEpochTimeToDateString(data.start),
  );
  const data = {
    labels,
    datasets: [
      ...
      {
        label: 'Link Clicked',
        data: marketingMessageInsightsData.map((data) =>
          data.clicked && data.clicked.length > 0 ? data.clicked[0].count : 0,
        ),
  ...
  const totalSent = marketingMessageInsightsData.reduce(
    (accumulator, current) => accumulator + current.sent,
    0,
  );
  const totalDelivered = marketingMessageInsightsData.reduce(
    (accumulator, current) => accumulator + current.delivered,
    0,
  );
  const totalRead = marketingMessageInsightsData.reduce(
    (accumulator, current) => accumulator + current.read,
    0,
  );
  const totalClicks = marketingMessageInsightsData.reduce(
    (accumulator, current) =>
      current.clicked && current.clicked.length > 0
        ? accumulator + current.clicked[0].count
        : accumulator,
    0,
  );
            ...
            <Section title={section.title} key={section.title}>
              {
                <div className={styles.rowContainer}>
                  {marketingMessageInsightsData.length === 0 && (
                    <SkeletonChart size={'big'} />
                  )}
                  {marketingMessageInsightsData.length > 0 && (
                    ...
                    <div>
                      ...
                      <p className={styles.metricContainer}>
                        <span className={styles.icon}>
                          {AntDesignIcons['AiOutlineSend']()}
                        </span>
                        <span className="metric-value">
                          {totalSent} Messages Sent
                        </span>
                        <span className={styles.icon}>
                          {AntDesignIcons['AiOutlineDeliveredProcedure']()}
                        </span>
                        <span className="metric-value">
                          {totalDelivered} Messages Delivered
                        </span>
                        <span className={styles.icon}>
                          {AntDesignIcons['AiOutlineRead']()}
                        </span>
                        <span className="metric-value">
                          {totalRead} Messages Read
                        </span>
                        <span className={styles.icon}>
                          {AntDesignIcons['AiFillForward']()}
                        </span>
                        <span className="metric-value">
