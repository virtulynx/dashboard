      ...
      if (data) {
        const activeAdAccounts = data.filter((account) => {
          return (
            ...
            account.insights.data.length > 0
          ); // Active Ad Accounts only with insights
        });

        if (activeAdAccounts.length === 0) {
          dispatchError(
            { details: errors.noAdInsightsAvailable },
            insightsObj.error,
          );
        }
        ...
        dispatch({
          type: insightsObj.stateNameAdAccounts,
          payload: activeAdAccounts,
        });
      } else if (error) {
  ...
  const getInsights = async (insightsObj, video) => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        if (video) {
          dispatch({
            type: insightsObj.stateName,
            payload: { ...video, insights: response.data },
          });
        } else {
