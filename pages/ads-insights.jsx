...
const AdsInsights = ({
  ...
  const adsInsightsAdAccounts = useSelector(
    (state) => state.adsInsightsAdAccounts,
  );
  const adsInsightsAccountsData = useSelector(
    (state) => state.adsInsightsAccount,
  );
  const adsInsightsCampaignData = useSelector(
    (state) => state.adsInsightsCampaigns,
  );
  const accountCurrency =
    adsInsightsAccountsData &&
    adsInsightsAccountsData[0] &&
    adsInsightsAccountsData[0].currency
      ? adsInsightsAccountsData[0].currency
      : '';
                    ...
                    {section.charts.map((el) => {
                      return (
                        <AdsDashboardChart
                          ...
                          insights={selectAdsInsights(el.metrics, {
                            account: adsInsightsAccountsData,
                            campaigns: adsInsightsCampaignData,
                          })}
                          title={el.title}
              ...
              <FormControl component="fieldset">
                ...
                <FormGroup aria-label="position" row>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={filters.messenger_only}
