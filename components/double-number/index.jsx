...
const DoubleNumber = ({ datasets, labels }) => {
  ...
  return (
    <div>
      <p className={styles.metricContainer}>
        <span className={`metric-value-${datasets[0].label}`}>
          {getLastNumber(datasets[0].data)}
        </span>
        <span> {getLabel(datasets[0].label)}</span>
      </p>
      {datasets.length > 1 && (
        <p className={styles.metricContainer}>
          <span className={`metric-value-${datasets[1].label}`}>
            {getLastNumber(datasets[1].data)}
          </span>
