...
const DashboardChart = ({
  ...
  const formatDataForPieChart = (datasets) => {
    ...
    completeDataset.backgroundColor = generateRandomColorArray(
      completeDataset.data.length,
    );
  ...
  return (
    ...
    >
      {chartData.datasets.length > 0 && chartData.datasets[0].loaded && (
        <Card shadow={false} id={`chart-${chartData.name}`}>
          <h1 className={styles.chartTitle}>
            {' '}
            {capitalizeAll(chartData.title)}
          </h1>
          <div
            className={`${styles.metricName} ${wrapMetricName ? styles.wrap : ''}`}
          ...
          {type === types.bar && (
            <Bar data={chartData} options={config.barChart} />
          )}
          {type === types.pie && (
            <Pie
              data={chartData}
              options={config.pieChart}
              height={250}
              width={400}
            />
          )}
          {type === types.singleNumber && (
            <SingleNumber data={chartData} icons={icons} labels={labels} />
          )}
          {type === types.doubleNumber && (
            <DoubleNumber
          ...
          {type === types.aggregateByProperty && (
            <AggregateByProperty
              datasets={chartData.datasets}
              plural={plural}
              icons={icons}
          ...
          {isShown && type !== types.singleNumber && (
            <Tooltip info={tooltipInfo} />
          )}
        </Card>
      )}
      {errorMessage && (
        <ErrorCard icon="AiFillInfoCircle" error={errorMessage} />
      )}
