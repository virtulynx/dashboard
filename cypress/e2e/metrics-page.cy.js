                ...
                switch (metric.name) {
                  case 'page_fans':
                    ...
                    cy.get('#chart-page_fans .metric-value').should(($p) => {
                      expect($p).to.contain(
                        abbreviateNumber(lastElement[0].value),
                      );
                    });
                    break;
                  case 'page_positive_feedback_by_type':
                    ...
                    cy.get('#chart-page_positive_feedback_by_type').should(
                      ($card) => {
                        expect($card.find('.like .metric-value')).to.contain(
                          abbreviateNumber(like),
                        );
                        expect($card.find('.comment .metric-value')).to.contain(
                          abbreviateNumber(comment),
                        );
                        expect($card.find('.link .metric-value')).to.contain(
                          abbreviateNumber(link),
                        );
                        expect($card.find('.other .metric-value')).to.contain(
                          abbreviateNumber(other),
                        );
                      },
                    );
                    break;
