...
const getLast30DaysInterval = () => {
  ...
  const since = new Date(new Date().setDate(today.getDate() - 30))
    .toISOString()
    .split('T')[0];
  return { since, until };
};
...
const getLastNDaysInterval = (num) => {
  const today = new Date();
  const until = today.toISOString().split('T')[0];
  const since = new Date(new Date().setDate(today.getDate() - num))
    .toISOString()
    .split('T')[0];
  return { since, until };
};
...
export {
  ...
  formatEpochTimeToDateString,
  getLast30DaysInterval,
  getLastNDaysInterval,
  getLast30DayEpoc,
